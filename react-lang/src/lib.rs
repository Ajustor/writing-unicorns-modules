...
const TYPE_KEYWORDS: &[&str] = &[
    ...
    "FC", "ReactNode", "ReactElement", "JSX", "RefObject", "MutableRefObject",
    "Dispatch", "SetStateAction", "MouseEvent", "KeyboardEvent", "ChangeEvent",
    "FormEvent", "CSSProperties", "HTMLElement", "HTMLDivElement",
    "HTMLInputElement", "HTMLButtonElement",
];

// React hooks highlighted as macros for visual distinction
...
pub fn tokenize_line(line: &str) -> Vec<Token> {
    ...
    while i < len {
        ...
        // JSX comment: {/* ... */} — treat rest of line as comment when we hit {/*
        if line[offset..].starts_with("{/*") {
            tokens.push(Token { text: line[offset..].to_string(), kind: TokenKind::Comment });
            break;
        }
        ...
        // Word (identifier, keyword, hook, type, function)
        if chars[i].is_alphabetic() || chars[i] == '_' {
            ...
            let kind = if REACT_HOOKS.contains(&word.as_str()) {
                // Hooks are functions; mark as macro for distinction
                TokenKind::Macro
...
pub fn hover_info(word: &str, file_content: &str) -> Option<String> {
    let fn_patterns = [
        ...
        format!("const {}: FC", word),
        format!("const {}: React.FC", word),
    ];
    let type_patterns = [
...
mod tests {
    ...
    #[test]
    fn test_keyword() {
        let tokens = tokenize_line("const App = () => {");
        assert!(tokens.iter().any(|t| t.text == "const" && t.kind == TokenKind::Keyword));
    }

    #[test]
    fn test_hook() {
        let tokens = tokenize_line("const [count, setCount] = useState(0);");
        assert!(tokens.iter().any(|t| t.text == "useState" && t.kind == TokenKind::Macro));
    }

    #[test]
    fn test_comment() {
        let tokens = tokenize_line("// a comment");
    ...
    #[test]
    fn test_jsx_comment() {
        let tokens = tokenize_line("{/* jsx comment */}");
        assert_eq!(tokens.len(), 1);
        assert_eq!(tokens[0].kind, TokenKind::Comment);
    }
    ...
    fn test_type_keyword() {
        let tokens = tokenize_line("const f: FC = () => null;");
        assert!(tokens.iter().any(|t| t.text == "FC" && t.kind == TokenKind::KeywordType));
    }
}
