...
/// Returns the LSP server command for JavaScript: typescript-language-server
...
mod tests {
    ...
    fn test_template_literal() {
        let tokens = tokenize_line("const s = `hello world`;");
        assert!(tokens
            .iter()
    ...
    fn test_arrow_function() {
        let tokens = tokenize_line("const add = (a, b) => a + b;");
        assert!(tokens
            .iter()
            .any(|t| t.text == "const" && t.kind == TokenKind::Keyword));
    }
    ...
    #[test]
    fn test_import() {
        let tokens = tokenize_line("import { foo } from 'bar';");
        assert!(tokens
            .iter()
            .any(|t| t.text == "import" && t.kind == TokenKind::Keyword));
        assert!(tokens
            .iter()
            .any(|t| t.text == "from" && t.kind == TokenKind::Keyword));
    }

    #[test]
    fn test_async_await() {
        let tokens = tokenize_line("async function fetch() {");
        assert!(tokens
            .iter()
            .any(|t| t.text == "async" && t.kind == TokenKind::Keyword));
    }
}
